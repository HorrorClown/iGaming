...
--[[
    --//
    --|| Woltlab Community Framework Plugin: jCoins
    --\\
 ]]

function Cwbbc:getCoins()

end

function Cwbbc:setCoins()

end

function Cwbbc:addCoins()

end

function Cwbbc:removeCoins()
...
function Cwbbc:message(sMessage)
    ...
    debugOutput(("[%s@%s]: %s"):format(self.sUser, self.sDBName, sMessage))
end
...
function Cwbbc:debugOutput(tMessages)
    ...
    for sIndicator, sMessage in pairs(tMessages) do
        debugOutput(("[iConnect][%s] %s"):format(tostring(sIndicator), tostring(sMessage)))
